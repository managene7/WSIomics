...
parser.add_argument('--feature_dir', type=str, default="FEATURES_DIRECTORY",
                    help='directory of feature extraction results')
...
encoding_size = args.embed_dim #1024   <-----------------------
...
print ("\n\nLabel file info: ", label_dict_cont,"\n\n")

if args.task == 'task_1_tumor_vs_normal':
    args.n_classes=len(label_dict_cont.keys())
    dataset = Generic_MIL_Dataset(csv_path = args.classification_csv, #'dataset_csv/tumor_vs_normal_dummy_clean.csv', 
                            data_dir= args.feature_dir, #os.path.join(args.data_root_dir, 'tumor_vs_normal_resnet_features'),
                            shuffle = False, 
                            seed = args.seed, 
                            print_info = True,
    ...
    args.n_classes=len(label_dict_cont.keys())
    dataset = Generic_MIL_Dataset(csv_path = args.classification_csv, #'dataset_csv/tumor_subtyping_dummy_clean.csv',
                            data_dir= args.feature_dir, # os.path.join(args.data_root_dir, 'tumor_subtyping_resnet_features'),
                            shuffle = False, 
                            seed = args.seed, 
                            print_info = True,
...
# args.results_dir = os.path.join(args.results_dir, str(args.exp_code) + '_s{}'.format(args.seed)) # <==== modified
if not os.path.isdir(args.results_dir):
    os.mkdir(args.results_dir)
