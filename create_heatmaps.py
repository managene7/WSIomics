...
parser.add_argument('--config_file', type=str, default="heatmap_config.yaml", help="Heatmap config file name. default: heatmap_config.yaml")
parser.add_argument('--heatmap_out_folder', type=str, default="HEATMAP_OUTPUT_FILES", help="Folder name for heatmap outputs")
...
if __name__ == '__main__':
    ...
    config_path = args_0.config_file
    config_dict = yaml.safe_load(open(config_path, 'r'))
    config_dict = parse_config_dict(args_0, config_dict)
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        os.makedirs(args_0.heatmap_out_folder, exist_ok=True)
        if data_args.process_list is not None:
            #process_stack.to_csv('heatmaps/results/{}.csv'.format(data_args.process_list.replace('.csv', '').split('/')[-1]), index=False)
            process_stack.to_csv(args_0.heatmap_out_folder+'/{}.csv'.format(data_args.process_list.replace('.csv', '').split('/')[-1]), index=False)
        else:
            #process_stack.to_csv('heatmaps/results/{}.csv'.format(exp_args.save_exp_code), index=False)
            process_stack.to_csv(args_0.heatmap_out_folder+'/{}.csv'.format(exp_args.save_exp_code), index=False)
